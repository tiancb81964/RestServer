...
public class TenantTree {
	...
	/**
	 * Get the orgin tenantID which was passed to Constructor {@link #TenantsTree(String)}
	 * @return
	 */
	...
	private void initParent(TenantTreeNode node) {
		...
		if (parent != null) {
			TenantTreeNode pNode = new TenantTreeNode(parent.getId(), parent.getName(), parent.getDescription(), null);
			pNode.setChildren(new ArrayList<TenantTreeNode>(){{add(node);}});
	...
	private void initChildren(TenantTreeNode tenantNode) {
		...
		if (children != null && !children.isEmpty()) {
			for (Tenant child : children) {
				TenantTreeNode subNode = new TenantTreeNode(child.getId(), child.getName(), child.getDescription(), tenantNode);
	...
	public static class TenantTreeNode{
		...
		public TenantTreeNode(String id, String name, String description, TenantTreeNode parent){
			this.id = id;
			this.name = name;
			this.parent = parent;
		...
		/**
		 * Get the level of current node. eg: root node is in level 1 , 2nd layer of nodes corresponds to  level 2, etc.
		 * @return
		 */
		public int getLevel(){
		...
		public String toString(){
			return "id:" + this.id + "	name:" + this.name + "	parentID:" + this.parentToString() + "	children:" + this.childrenToString();
		}
	}
