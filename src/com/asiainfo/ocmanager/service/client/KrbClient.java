...
import com.asiainfo.ocmanager.service.exception.KerberosAdminAuthenticationException;
import com.asiainfo.ocmanager.service.exception.KerberosKDCConnectionException;
import com.asiainfo.ocmanager.service.exception.KerberosOperationException;
import com.asiainfo.ocmanager.service.exception.KerberosRealmException;
import com.asiainfo.ocmanager.utils.KerberosConfiguration;
import com.asiainfo.ocmanager.utils.ShellCommandUtil;

/**
 * Java Client for manipulate MIT Kerberos. Including the following: 1) Create
 * krb principal 2) Remove krb principal 3) Generate krb keytab and keytab file
 *
...
public class KrbClient {
	 ...
	 *
	 * @param query
	 *            a String containing the query to send to the kdamin command
	 * @return a ShellCommandUtil.Result containing the result of the operation
	 * @throws KerberosKDCConnectionException
	 *             if a connection to the KDC cannot be made
	 * @throws KerberosAdminAuthenticationException
	 *             if the administrator credentials fail to authenticate
	 * @throws KerberosRealmException
	 *             if the realm does not map to a KDC
	 * @throws KerberosOperationException
	 *             if an unexpected error occurred
	 */
	public ShellCommandUtil.Result invokeKAdmin(String query) throws KerberosOperationException {
	 ...
	 * <p/>
	 * This implementation creates a query to send to the kadmin shell command
	 * and then interrogates the result from STDOUT to determine if the
	 * operation executed successfully.
	 *
	 * @param principal
	 *            a String containing the principal add
	 * @param password
	 *            a String containing the password to use when creating the
	 *            principal
	 * @return an Integer declaring the generated key number
	 * @throws KerberosOperationException
	 *             if an unexpected error occurred
	 */
	public void createPrincipal(String principal, String password) throws KerberosOperationException {
		if (StringUtils.isEmpty(principal)) {
			...
			// Create the kdamin query: add_principal <-randkey|-pw <password>>
			// [<options>] <principal>
			ShellCommandUtil.Result result = invokeKAdmin(
					String.format("add_principal -pw \"%s\" %s", password, principal));
	...
	/**
	 * Test to see if the specified principal exists in a previously configured
	 * MIT KDC
	 * <p/>
	 * This implementation creates a query to send to the kadmin shell command
	 * and then interrogates the result from STDOUT to determine if the presence
	 * of the specified principal.
	 *
	 * @param principal
	 *            a String containing the principal to test
	 ...
	 *
	 * @param principal
	 *            a String containing the principal to remove
	 ...
	 *            principal
	 * @param keyNumber
	 *            a Integer indicating the key number for the keytab entries
	 * @return the created Keytab
	 * @throws KerberosOperationException
	 */
	public Keytab createKeyTab(String principal, String password, Integer keyNumber) throws KerberosOperationException {
		if (StringUtils.isEmpty(principal)) {
	 ...
	 * 
	 * @param keytab
	 *            the Keytab containing the data to add to the keytab file
	 * @param keytabFilePath
	 *            a File containing the absolute path to where the keytab data
	 *            is to be stored
	 * @return true if the keytab file was successfully created; false otherwise
	 */
	public boolean createKeyTabFile(Keytab keytab, String keytabFilePath) throws KerberosOperationException {
	 ...
	 * @param keyNumber
	 *            a Integer indicating the key number for the keytab entries
	 * @return a keytab string using the base64 encode if keytab was
	 *         successfully created; empty string otherwise
	 */
