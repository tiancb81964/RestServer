...
//		for(PartitionMetadata partitionMeta : toList(meta.partitionsMetadata())){
...
//		return partitions;
	...
	/**
	 * Kafka partition, consist of parent topic, partitionid
	 * and allocated host of this partition.
	 * @author EthanWang
	 *
	 */
	...
	private static class MBeanName{
		...
		public static ObjectName name(String topic, int partition) throws MalformedObjectNameException{
			return new ObjectName(OBJECTSTR.replace("{%TOPIC}", topic).replace("{%PARTITION}", String.valueOf(partition)));
		}
	}
